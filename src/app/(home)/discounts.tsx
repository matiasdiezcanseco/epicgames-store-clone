    ...
    <div className="grid gap-6 grid-cols-3 pt-8">
        {dealsOfTheWeek.map((deal) => (
            <div key={deal.title} className="flex flex-col gap-6">
                <Image className="rounded-2xl overflow-hidden" src={deal.imageUrl} 
                alt={deal.title}
                height={854 / 1.78}
                    width={854}
                ...
                <div className="flex gap-2 items-center">
                    <p className="bg-sky-600 rounded-md py-1 px-2 text-xs">{deal.discountPercentage}%</p>
                    <p className="line-through text-text-secondary text-lg">USD {deal.price}</p>
                    <p className="text-lg">USD {deal.discountPrice}</p>
            </div>
            </div>
        ))}
         <div className="flex flex-col gap-6">
                <Image className="rounded-2xl overflow-hidden" src="/deals-week/sales-special.avif" 
                alt="Deal's Special"
                height={854 / 1.78}
                    width={854}
                    style={{ objectFit: 'contain' }} />
                <p className="text-xl">Check out all the deals for this week.</p>
                <Link href="/" className="text-xl underline decoration-background-primary-highlight underline-offset-8">Browse</Link>
            </div>
    </div>
    </>
